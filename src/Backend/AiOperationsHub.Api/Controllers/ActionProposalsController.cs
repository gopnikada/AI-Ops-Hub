...
using AiOperationsHub.Application.Actions.Commands.SelectJiraIssueEditTarget;
...
namespace AiOperationsHub.Api.Controllers
{
    ...
    public sealed class ActionProposalsController : ControllerBase
    {
        ...
        /// <summary>
        /// Starts Jira issue edit proposal preparation. Returns a proposal for a unique match, or candidates when selection is required.
        /// </summary>
        ...
        /// <summary>
        /// Selects one Jira issue candidate and creates the edit proposal preview.
        /// </summary>
        [HttpPost("jira-issue-edit/select-target")]
        [Authorize(Policy = AuthorizationPolicies.CanCreateProposals)]
        [ProducesResponseType(typeof(ActionProposalResponse), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ValidationProblemDetails), StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<ActionProposalResponse>> SelectJiraIssueEditTarget(
            [FromBody] SelectJiraIssueEditTargetRequest request,
            CancellationToken cancellationToken)
        {
            var proposal = await _sender.Send(
                new SelectJiraIssueEditTargetCommand
                {
                    RequestedByUserId = User.GetRequiredUserId(),
                    CorrelationId = Guid.NewGuid(),
                    ConversationId = request.ConversationId,
                    ProjectKey = request.ProjectKey,
                    IssueReference = request.IssueReference,
                    SelectedIssueKey = request.SelectedIssueKey,
                    Summary = request.Summary,
                    Description = request.Description,
                    Assignee = request.Assignee,
                    Status = request.Status
